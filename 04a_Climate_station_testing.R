...
source("functions/dl_climate_data_testing.R")
...
for (i in Regional_group_list) {
  ...
  final_results4_2 <- final_results4 %>%
    dplyr::select(station_name, climate_lon, climate_lat, Well, station_id,
                  start_date, end_date, temp_precentcomplete, precip_precentcomplete )


  filename = paste0(output_path, "/Complete_climate_station_list_", i, ".csv")
  write_csv(final_results4_2, filename )

  station_names <- final_results4_2 %>%
    dplyr::select(climate_infilled_1 = station_id, Climate_station_me = station_name) %>%
    unique()
  ...
  climate_data_testing_compiled_DT_comb <- bind_rows(climate_data_testing_compiled_DT_count1,
                                                     climate_data_testing_compiled_DT_count2)
  ...
  climate_data_testing_compiled_DT_comb2 <- climate_data_testing_compiled_DT_comb %>%
    ...
    mutate(temp_clim1 = sub("\\..*", "", climate_infilled_1)) %>% # add temp column to get name of first stn listed
    left_join(station_names, by = join_by(temp_clim1 == climate_infilled_1)) %>%
    dplyr::select(Well, Region, climate_infilled_1, Climate_station_me,
                  everything(), -temp_clim1)
