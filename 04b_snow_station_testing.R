...
source("Groundwater Level Forecasting Toolkit Phase 2 V2.0/functions/dl_snow_data_testing.R")
...
results <- lapply(1:nrow(pgown_well_info_proj), function(i) {
  ...
  stations_within_buffer <- find_stations_within_buffer(well, snow_stations, buffer_distance)
  return(stations_within_buffer)
})
...
snow_data_raw <- lapply(snow_station_list, function(stn) {
  get_aswe_databc(station_id = stn, get_year = "All", parameter = "swe", timestep = "daily")
}) %>%
  bind_rows() %>%
