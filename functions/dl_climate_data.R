dl_climate_data <- function(pgown_well_info, data_location) {
  ...
  message("Starting climate data download...")
  ...
  station_order <- pgown_well_info %>%
    dplyr::select(c("Well","Climate_station_Id", "Climate_Infilled_id", "Climate_secondary_Infilled",
                    "Climate_tertiary_Infilled", "Climate_quaternary_Infilled")) %>%
    distinct()
  ...
  for (i in Well_list){
    ...
    climate_filled <- left_join(climate_filled, ranks, by = join_by(station_id))
  ...
  message("...climate data download done!")
