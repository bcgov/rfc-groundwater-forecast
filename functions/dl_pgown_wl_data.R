dl_pgown_wl_data <- function(pgown_well_info, data_location) {
  ...
  message("Starting PGOWN data download...")
  ...
  pgown_data <- lapply(well_list, function(wellname) {
    ...
    full_join(pgown_well_info, by = join_by(Well)) %>%
    mutate(Date = as.Date(Time)) %>%
    group_by(Well, Date, Snow_influenced) %>%
    summarise(groundwater = mean(Value, na.rm = TRUE),
              groundwater_min = max(Value, na.rm = TRUE),
              .groups = "keep") %>%
    ungroup()

  save(pgown_data, file = paste0(data_location, "pgown_data.RData"))

  message("...PGOWN data download done!")
